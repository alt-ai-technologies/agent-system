...
GIT_TOOLS="Bash(git add:*) Bash(git rm:*) Bash(git commit:*) Bash(git diff:*) Bash(git log:*) Bash(git status:*) Bash(git branch:*) Bash(git checkout:*) Bash(git show:*) Bash(git stash:*) Bash(git rev-parse:*)"

# Git with -C <path> — same commands, allowed when agent operates on external repos
GIT_C_TOOLS="Bash(git -C * add *) Bash(git -C * rm *) Bash(git -C * commit *) Bash(git -C * diff *) Bash(git -C * log *) Bash(git -C * status *) Bash(git -C * branch *) Bash(git -C * checkout *) Bash(git -C * show *) Bash(git -C * stash *) Bash(git -C * rev-parse *)"

...
GIT_C_TOOLS="Bash(git -C * add *) Bash(git -C * rm *) Bash(git -C * commit *) Bash(git -C * diff *) Bash(git -C * diff) Bash(git -C * log *) Bash(git -C * log) Bash(git -C * status *) Bash(git -C * status) Bash(git -C * branch *) Bash(git -C * branch) Bash(git -C * checkout *) Bash(git -C * show *) Bash(git -C * stash *) Bash(git -C * rev-parse *) Bash(git -C * fetch *) Bash(git -C * fetch) Bash(git -C * merge *) Bash(git -C * merge-base *) Bash(git -C * rev-list *) Bash(git -C * ls-tree *) Bash(git -C * merge-tree *)"
